...
class Queue(threading.Thread):
    ...
    def add_to_queue(self, building):
        ...
        if building.supply == 'supply1':
            ...
            self.buildings_queue.append(queue_item)
        elif building.supply == 'supply12':
            queue_item = QFusion(building, level)
            self.buildings_queue.append(queue_item)
        elif building.supply == 'supply22':
            queue_item = QMetallStorage(building, level)
            self.buildings_queue.append(queue_item)
        elif building.supply == 'supply23':
            queue_item = QKrystalStorage(building, level)
            self.buildings_queue.append(queue_item)
        elif building.supply == 'supply24':
            queue_item = QDeuteriumStorage(building, level)
            self.buildings_queue.append(queue_item)
        else:
    ...
    def upgrade_building(self, building_queue_item):
        """
        :param building_queue_item: build_queue item to upgrade
    ...
    def building(self):
        if self.current_remaining_time > 0:
            self.is_building = True
            self.current_remaining_time -= 1
            t = threading.Timer(1, self.building)
            t.setDaemon(True)
            t.start()
        else:
            self.is_building = False
    ...
    def find_current_constructing_building_info(self):
        ...
        self.planet.page['resource'].refresh_content()
        extract_building_time = re.findall(FIND_ACTUAL_BUILDING_TIME, self.planet.page['resource'].content)

        numbers = []
        for number in extract_building_time:
